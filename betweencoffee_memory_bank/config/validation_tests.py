    ...
    def test_config_file_valid(self):
        ...
        try:
            ...
            if missing_fields:
                ...
                self.passed_tests += 1
                print(
                    f"✅ 配置文件格式正確 (版本: {config.get('memory_bank', {}).get('version', '未知')})"
                )
            ...
            try:
                ...
                if file_size_mb > max_size_mb:
                    problematic_files.append(
                        (file_path.name, f"太大: {file_size_mb:.1f}MB")
                    )
                elif file_size < min_size_kb * 1024:
                    problematic_files.append(
                        (file_path.name, f"太小: {file_size_kb:.1f}KB")
                    )
                ...
                for link_text, link_url in links:
                    ...
                    if link_url.startswith("./") or link_url.endswith(".md"):
                        ...
                        if not target_path.exists():
                            broken_links.append(
                                f"{file_path.name}: 鏈接 '{link_text}' -> '{link_url}' 不存在"
                            )
...
def main():
    ...
    parser.add_argument(
        "--exit-on-error", action="store_true", help="發現錯誤時退出碼為1"
    )
