...
class MemoryBankLoader:
    ...
    def load_config(self) -> Optional[Dict]:
        ...
        try:
            ...
            print(
                f"✅ 已載入 Memory Bank 配置 (版本: {config.get('memory_bank', {}).get('version', '未知')})"
            )
            return config
    ...
    def _check_file_ages(
        self, file_names: List[str], max_age_days: int
    ...
    def load_memory_bank_files(self, config: Dict) -> Dict[str, str]:
        ...
        summarize_large = config.get("auto_load_config", {}).get(
        ...
        for file_name in load_order:
            ...
            try:
                ...
                if estimated_tokens > max_tokens and summarize_large:
                    print(
                        f"📄 文件較大 ({estimated_tokens} tokens)，創建摘要: {file_name}"
                    )
    ...
    def generate_context_prompt(
        self, config: Dict, loaded_files: Dict[str, str]
    ) -> str:
        """
        生成上下文提示詞
        ...
        print(
            f"📊 上下文提示詞生成完成: {len(full_prompt)} 字符, ~{estimated_tokens} tokens"
        )
...
def main():
    ...
    parser.add_argument(
        "--validate-only", action="store_true", help="僅驗證不生成上下文"
    )
