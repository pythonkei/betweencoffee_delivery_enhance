...
> **最後更新**: 2026年9月15日（正式庫資料落差回補完成；sake-mv 資源集中 + 咖啡詳情頁氣泡微調；記憶庫瘦身：歷史日誌已封存，本檔僅現況）
...
- 2026-09-15 咖啡氣泡「出現位置」全變數化：三顆氣泡（p-1/p-2/p-3）在照片上的落點抽成 CSS 變數（`--bc-cb-p1/p2/p3-top/-left` ＋手機（≤767.98px）`-top-sm`），預設＝原本寫死的值 → 外觀零變化；彈出動畫效果與原站完全相同（僅把 `--cb-in-x/y/s/r/dur/tdur/delay/ease/stamp-r` 數值變成可覆寫）；1280px CDP 實測相對落點（bottom 59.29/77.03/33.30%、left −52.2/337.8/333.0px）與尺寸與改動前逐項相同、覆寫 `--bc-cb-p1-top` 即時生效；版號 bc-coffee-comment.css ?v=20260915b
