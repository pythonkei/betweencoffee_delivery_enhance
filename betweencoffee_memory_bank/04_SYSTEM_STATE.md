...
- 2026-09-15 咖啡氣泡落點全變數化＋逐款可微調：三顆氣泡的落點抽成 CSS 變數，並新增 `bubble_dx_1/2/3`、`bubble_dy_1/2/3`（migration 0070）＋ Admin 欄位組「氣泡落點微調」，**每一款咖啡**可自行用偏移量調整（單位＝照片寬／高的 %，正值往右／往下；留空＝預設位置）；彈出動畫效果與原站完全相同（僅抽成 `--cb-in-*` 變數）；1280px CDP 實測偏移 0 時與改動前逐項相同、dy_1=-10 → 上移 10.00%、dx_3=4 → 右移 4%×寬；版號 bc-coffee-comment.css ?v=20260915c
