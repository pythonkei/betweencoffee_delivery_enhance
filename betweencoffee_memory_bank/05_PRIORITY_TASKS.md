...
2. **氣泡文字垂直置中**（✅ 完成 2026-09-14）：使用者回報 `blowing-pattern02a.svg` 未與文字中心對齊 → 診斷為 SVG 本身水平已精確置中，真因是文字在氣泡內的垂直位置；視覺基準取**氣泡面積質心**（小尾巴會讓 ink bbox 中心偏低 ~3.5px）；只重新分配 `.text` 上下內距（合計不變）：pattern-1 上 1.1803fs / 下 1.6497fs、pattern-5 上 0.9318fs / 下 1.3972fs（與原始值僅差 0.2~0.35px → 原設計已接近最佳）；最終 4 斷點實測 ≤ 0.44px
