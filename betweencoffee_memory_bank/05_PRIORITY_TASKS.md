...
## 近期優先
1. **咖啡詳情頁 staff-comment 氣泡**（✅ 完成 2026-09-13，2026-09-14 微調＋DB 化＋幾何可設定）：三顆氣泡文字由 `CoffeeItem.staff_comment_1/2/3` 驅動（Admin 逐款改字、留空不顯示）＋ 第三顆（pattern-2 / `blowing-pattern03a.svg` / p-2 右側下半）＋ **定位幾何每款可設定**（`bubble_safe_left/right`、`bubble_photo_ratio`、`bubble_scale`；留空時存檔以 Pillow 自動量測去背 PNG，換圖後清空再存即重新量測）→ 直式、橫式（Black Blend 1200×840）與其他比例（Flat White 840×1200）皆可用；內距依字數 `--bc-cb-n` 自動置中＋ **氣泡落點全變數化＋逐款可微調**（✅ 2026-09-15：落點抽成 CSS 變數 `--bc-cb-p1/p2/p3-top/-left`＋手機 `-top-sm`；新增 `bubble_dx/dy_1/2/3` 欄位（migration 0070）→ 每款咖啡可在 Admin 填偏移量（% 照片寬/高）微調自己的落點；彈出動畫效果與原站相同；CDP 實測偏移 0 時外觀與改動前逐項相同、設値後位移精確等於設定值）；版號 bc-coffee-comment.css ?v=20260915c、bc-coffee-comment.js ?v=20260914a
