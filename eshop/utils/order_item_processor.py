...
class OrderItemProcessor:
    ...
    def process_order_items(items: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for item in items:
            ...
            if not item_copy.get("image"):
                item_copy["image"] = OrderItemProcessor._get_default_image_url(
                    item_type
                )
    ...
    def prepare_order_data(
        order, queue_item=None, now=None, hk_tz=None, include_queue_info: bool = True
    ) -> Dict[str, Any]:
        """
        準備訂單數據用於顯示
        ...
        # 計算總價
        total_price = OrderItemProcessor.calculate_total_price(
            order, item_result["all_items"]
        )
        ...
        order_data = {
            ...
            "pickup_time_display": (
                pickup_time_info["text"] if pickup_time_info else "--"
            ),
            "pickup_time_choice": (
                order.pickup_time_choice
                if hasattr(order, "pickup_time_choice")
                else None
            ),
        }
        ...
        if now and hk_tz:
            created_at_hk = (
                order.created_at.astimezone(hk_tz)
                if order.created_at.tzinfo
                else timezone.make_aware(order.created_at, hk_tz)
            )
        ...
        if include_queue_info and queue_item:
            order_data.update(
                OrderItemProcessor._prepare_queue_info(queue_item, now, hk_tz)
            )
        ...
        if now and hk_tz:
            ...
            if queue_item.estimated_completion_time:
                ...
                queue_info["estimated_completion_time"] = (
                    estimated_completion_time_hk.strftime("%H:%M")
                )
                queue_info["estimated_completion_time_iso"] = (
                    estimated_completion_time_hk.isoformat()
                )

                # 計算剩餘時間
                remaining_seconds = max(
                    0, int((estimated_completion_time_hk - now).total_seconds())
                )
            ...
            if queue_item.estimated_start_time:
                ...
                queue_info["estimated_start_display"] = estimated_start_hk.strftime(
                    "%H:%M"
                )
    ...
    def prepare_ready_order_data(order, now=None, hk_tz=None) -> Dict[str, Any]:
        ...
        order_data = OrderItemProcessor.prepare_order_data(
            order, now=now, hk_tz=hk_tz, include_queue_info=False
        )
        ...
        if order.ready_at and hk_tz:
            ...
            if now and not order_data["is_beans_only"]:
                ...
                order_data["wait_display"] = (
                    f"{wait_minutes}分鐘前" if wait_minutes > 0 else "刚刚"
                )
