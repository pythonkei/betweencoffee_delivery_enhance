...
class CommonUtils:
    ...
    def create_api_response(
        ...
        if success:
            ...
            return api_error(
                message=message, status_code=status_code, details=error_details
            )

    @staticmethod
    def handle_exception_as_api_response(
        e: Exception, context: str = "unknown", operation: str = "unknown"
    ) -> JsonResponse:
        """
        將異常處理為API響應
        ...
        error_result = handle_error(
            error=e, context=context, operation=operation, log_level="error"
        )
    ...
    def serialize_order_basic(order) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "created_at": TimeFormatter.format_iso(
                    order.created_at, CommonUtils.HONG_KONG_TZ
                ),
                "created_at_display": TimeFormatter.format_for_display(
                    order.created_at, CommonUtils.HONG_KONG_TZ
                ),
                "ready_at": (
                    TimeFormatter.format_iso(order.ready_at, CommonUtils.HONG_KONG_TZ)
                    if order.ready_at
                    else None
                ),
                "ready_at_display": (
                    TimeFormatter.format_for_display(
                        order.ready_at, CommonUtils.HONG_KONG_TZ
                    )
                    if order.ready_at
                    else None
                ),
            ...
            return {"id": order.id if order else 0, "error": "序列化失敗"}
    ...
    @staticmethod
    def log_operation(
        module: str, operation: str, message: str, level: str = "info", **kwargs
    ):
        """
        統一日誌記錄
...
def api_response(
    success: bool, message: str, data: Any = None, **kwargs
) -> JsonResponse:
    """快捷函數：創建API響應"""
...
if __name__ == "__main__":
    ...
    class TestObj:
        def __init__(self):
            self.user = type(
                ...
                (),
                {
                    "profile": type("Profile", (), {"name": "John"})(),
    ...
    errors = common_utils.validate_required_fields(
