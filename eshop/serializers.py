    ...
    def serialize_order(order, include_queue_info=True, include_items=True):
        ...
        try:
            ...
            if order.estimated_ready_time:
                ...
                order_data["estimated_ready_time_display"] = format_time_for_display(
                ...
                if order.estimated_ready_time > now_hk:
                    ...
                    order_data["remaining_minutes"] = max(
                        0, int(diff.total_seconds() / 60)
                    )
                else:
        ...
        try:
            ...
            if queue_item.estimated_completion_time:
                ...
                if (
                    ...
                    and queue_item.estimated_completion_time > now_hk
                ):
                    diff = queue_item.estimated_completion_time - now_hk
                    queue_info["queue_wait_minutes"] = max(
                        0, int(diff.total_seconds() / 60)
                    )
                else:
            ...
            if queue_item.actual_start_time:
                queue_info["actual_start_time"] = (
                    queue_item.actual_start_time.isoformat()
                )
            if queue_item.actual_completion_time:
                queue_info["actual_completion_time"] = (
        ...
        for queue_item in queue_items:
            ...
            if include_order_info and queue_item.order:
                queue_data["order"] = OrderDataSerializer.serialize_order(
                    queue_item.order, include_queue_info=False, include_items=True
                )
