...
class OrderStatusManager:
        ...
        try:
            ...
            if manager.should_add_to_queue():
                ...
                if queue_result.get('success'):
                    ...
                    logger.error(f"订单 {order.id} 加入队列失败: {queue_result.get('message')}")
            else:
    ...
    def process_order_status_change(cls, order_id, new_status, staff_name=None):
        ...
        try:
            ...
            if time_result.get('success'):
                ...
                logger.warning(f"⚠️ 訂單狀態變化後時間計算有問題: {time_result.get('message')}")
    ...
    def confirm_offline_payment(cls, order_id, payment_method, staff_name='staff'):
        ...
        try:
            logger.info(f"🔄 員工 {staff_name} 開始確認 {method_label} 付款，訂單 #{order_id}")
            ...
            if has_coffee:
                order.status = 'waiting'
                logger.info(f"✅ {method_label} 訂單 #{order_id} 含咖啡，狀態設為 waiting")
            elif has_beans:
                order.status = 'ready'
                logger.info(f"✅ {method_label} 訂單 #{order_id} 純咖啡豆，狀態設為 ready")
            else:
                order.status = 'waiting'
                logger.warning(f"⚠️ {method_label} 訂單 #{order_id} 無商品類型，設為 waiting")
            
            # 3. 保存訂單
            order.save()
            logger.info(f"✅ {method_label} 訂單 #{order_id} 付款確認成功")
            ...
            if has_coffee:
                try:
                    from .queue_manager_refactored import CoffeeQueueManager
                    queue_manager = CoffeeQueueManager()
                    queue_result_data = queue_manager.add_order_to_queue(order)
                    
                    if queue_result_data.get('success'):
                        queue_item = queue_result_data['data']['queue_item']
                        position = queue_item.position
                        logger.info(f"✅ {method_label} 訂單 #{order_id} 已加入製作隊列，位置: {position}")
                        queue_result = {'added': True, 'position': position}
                    else:
                        logger.warning(f"⚠️ {method_label} 訂單 #{order_id} 加入隊列失敗: {queue_result_data.get('message')}")
            ...
            try:
                ...
                send_order_update(
                    ...
                    data={
                        ...
                        'payment_status': 'paid',
                        'status': order.status,
                        'message': f'{method_label} 付款已確認，訂單 #{order_id} 已進入製作流程'
                    }
                )
            ...
            if order.user:
                try:
                    ...
                    loyalty, created = CustomerLoyalty.objects.get_or_create(user=order.user)
                    points_earned = loyalty.add_points_from_order(order)
                    logger.info(f"✅ 用戶 {order.user.username} {method_label} 訂單 #{order.id} 獲得 {points_earned} 積分")
                except Exception as e:
    ...
    def _get_queue_display_text(self, queue_info):
        ...
        if wait_minutes < 1:
            ...
            wait_display = f"{wait_minutes}分钟"
        
        queue_display = f"队列位置: #{queue_position} | 预计等待: {wait_display}"
        
        # 队列消息 - 根据队列状态提供更准确的信息
        if queue_position == 1:
            if has_preparing_items:
                queue_message = '当前订单正在制作中，即将轮到您！'
            else:
                queue_message = '下一个就轮到您了！'
        elif queue_position <= 3:
            # 显示更详细的信息
            if preparing_items_count > 0:
                queue_message = f'前面有 {preparing_items_count} 个正在制作，{waiting_items_count} 个等待中'
            else:
                queue_message = f'前面还有 {queue_position - 1} 个订单等待中'
        else:
            if preparing_items_count > 0:
                queue_message = f'目前有 {preparing_items_count} 个正在制作，{waiting_items_count} 个等待中'
            else:
                queue_message = '目前订单较多，请耐心等候'
        
        # 剩余时间显示 - 更准确的描述
        if total_minutes < 1:
            remaining_display = "(即将完成)"
        elif total_minutes <= 5:
            remaining_display = f"(约{total_minutes}分钟后完成)"
        else:
    ...
    def process_payment_and_update_status(cls, order_id, payment_method="unknown"):
        ...
        try:
            ...
            queue_manager = CoffeeQueueManager()
            queue_result = queue_manager.add_order_to_queue(order)
            queue_item = queue_result['data']['queue_item'] if queue_result.get('success') else None
    ...
    def _get_queue_info(self):
        ...
        try:
            ...
            # 计算所有在制作中的订单（waiting + preparing）
            active_items = CoffeeQueue.objects.filter(
                status__in=['waiting', 'preparing'],
                position__lt=queue_item.position
            ).order_by('position')
            
            queue_position = active_items.count() + 1
            ...
            for item in active_items:
                if item.status == 'preparing':
                    ...
                    if item.estimated_completion_time:
                        now = timezone.now()
                        remaining = (item.estimated_completion_time - now).total_seconds() / 60
                        queue_wait_minutes += max(0, remaining)
                    else:
            ...
            return {
                ...
                'queue_wait_minutes': round(queue_wait_minutes, 1),
                'preparation_minutes': preparation_minutes,
                'total_minutes': round(total_minutes, 1),
                'active_items_count': active_items.count(),
                'has_preparing_items': active_items.filter(status='preparing').exists(),
                'waiting_items_count': active_items.filter(status='waiting').count(),
                'preparing_items_count': active_items.filter(status='preparing').count(),
            }
