...
class CacheManager:
    ...
    def get_cached_queue_data(
        cache_type: str, get_data_func: Callable[[], Any], **kwargs
    ) -> Any:
        """
        獲取緩存的隊列數據
        ...
        return CacheManager.get_cached_data(cache_key, get_data_func, timeout=timeout)
    ...
    def invalidate_order_caches(order_id: int) -> bool:
        ...
        try:
            ...
            cache_key = CacheManager.get_cache_key(
                CacheManager.CACHE_PREFIXES["order_details"], order_id=order_id
            )
