...
def monitor_performance():
    ...
    quick_today = OrderModel.objects.filter(
        is_quick_order=True, created_at__gte=today
    ).count()
    ...
    test_queries = [
        (
            "等待隊列查詢",
            lambda: list(
                CoffeeQueue.objects.filter(status="waiting").order_by("position")[:50]
            ),
        ),
        (
            "今日訂單查詢",
            lambda: list(OrderModel.objects.filter(created_at__gte=today)[:50]),
        ),
        (
            "快速訂單查詢",
            lambda: list(
                OrderModel.objects.filter(is_quick_order=True, payment_status="paid")[
                    :50
                ]
            ),
        ),
