...
def generate_health_report():
    ...
    # 今日訂單統計
    today_stats = OrderModel.objects.filter(created_at__gte=today).aggregate(
        total=Count("id"),
    ...
    queries = [
        (
            "等待隊列查詢",
            lambda: list(
                CoffeeQueue.objects.filter(status="waiting").order_by("position")[:20]
            ),
        ),
        (
            "今日快速訂單",
            lambda: list(
                OrderModel.objects.filter(
                    is_quick_order=True, created_at__gte=today, payment_status="paid"
                )[:20]
            ),
        ),
        (
            "支付狀態查詢",
            lambda: list(
                OrderModel.objects.filter(payment_status="paid", status="waiting")[:20]
            ),
        ),
    ]
    ...
    old_orders = OrderModel.objects.filter(
        status="waiting", payment_status="paid", created_at__lt=two_hours_ago
    ).count()
    ...
    quick_waiting = OrderModel.objects.filter(
        is_quick_order=True, status="waiting", payment_status="paid"
    ).count()
