...
class BaseApiView(View):
    ...
    def handle_exception(self, exception):
        ...
        return JsonResponse(
            ApiResponseFormatter.error(message=error_message, code=status_code),
            status=status_code,
        )
    ...
    def success_response(self, data=None, message="操作成功", **kwargs):
        """成功響應"""
        return JsonResponse(ApiResponseFormatter.success(data, message, **kwargs))

    def error_response(
        self, message="操作失敗", code=None, details=None, status=400, **kwargs
    ):
        """錯誤響應"""
        return JsonResponse(
            ApiResponseFormatter.error(message, code, details, **kwargs), status=status
        )
...
class OrderApiMixin:
    ...
    def serialize_order(self, order, include_queue_info=True, include_items=True):
        ...
        return OrderDataSerializer.serialize_order(
            order, include_queue_info=include_queue_info, include_items=include_items
        )
