...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--hours", type=int, default=1, help="處理多少小時前的訂單（默認1小時）"
        )
        parser.add_argument(
            "--dry-run", action="store_true", help="模擬運行，不實際修改數據"
        )
        parser.add_argument(
            "--force", action="store_true", help="強制清理，即使訂單狀態不允許"
        )
    ...
    def handle(self, *args, **options):
        ...
        old_queues = CoffeeQueue.objects.filter(
            status="waiting", created_at__lt=cutoff_time
        ...
        if dry_run:
            ...
            for queue in old_queues:
                self.stdout.write(
                    f"訂單 #{queue.order.id} - {queue.order.name} - 創建於 {queue.created_at}"
                )
            return
        ...
        for queue in old_queues:
            self.stdout.write(
                f"處理訂單 #{queue.order.id} - {queue.order.name} - 創建於 {queue.created_at}"
            )
            ...
            try:
                ...
                # 先檢查訂單是否允許標記為就緒
                if (
                    order.status not in ["pending", "waiting", "preparing"]
                    and not force
                ):
                    self.stdout.write(
                        self.style.WARNING(
                            f"  跳過：訂單狀態為 {order.status}，不允許標記為就緒"
                        )
                    )
                    continue
                ...
                if not queue.actual_start_time:
                    queue.actual_start_time = timezone.now() - timedelta(
                        minutes=queue.preparation_time_minutes or 5
                    )
                ...
                result = OrderStatusManager.mark_as_ready_manually(
                    order_id=order.id, staff_name="cleanup_queue_command"
                )
                ...
                self.stdout.write(self.style.ERROR(f"  處理失敗：{str(e)}"))
        ...
        if failed > 0:
            self.stdout.write(self.style.WARNING(f"有 {failed} 個訂單處理失敗"))
        ...
        try:
            ...
            if time_result.get("success"):
                ...
                self.stdout.write(self.style.WARNING("⚠️ 隊列時間重新計算可能不完整"))
        except Exception as e:
