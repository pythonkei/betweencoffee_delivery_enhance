...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--batch-size", type=int, default=100, help="每批處理的訂單數量"
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            payment_status_counts = OrderModel.objects.values(
                "payment_status"
            ).annotate(count=Count("id"))
            self.stdout.write("支付狀態分布:")
            for stat in payment_status_counts:
                self.stdout.write(
                    f"    {stat['payment_status']}: {stat['count']} 個訂單"
                )
        ...
        self.stdout.write("\n建議後續步驟:")
        self.stdout.write(
            "1. 運行清理命令: python manage.py cleanup_payment_references"
        )
