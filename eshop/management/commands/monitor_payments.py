...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"找到 {timeout_orders.count()} 個超時訂單（寬限期: {grace_minutes}分鐘）"
        )
        ...
        for order in timeout_orders:
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f'  取消失敗: {result.get("message", "未知錯誤")}'
                        )
                    )
                ...
                self.stdout.write(self.style.ERROR(f"  處理異常: {str(e)}"))

        if cancelled > 0:
            self.stdout.write(self.style.SUCCESS(f"成功取消 {cancelled} 個超時訂單"))
        if failed > 0:
        ...
        try:
            ...
            self.stdout.write(self.style.WARNING(f"清理隊列項目時出現錯誤: {str(e)}"))
