...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING(f'模式: {"🔍 預覽 (dry-run)" if dry_run else "🔧 實際執行"}'))
        self.stdout.write(self.style.WARNING('=' * 70))
        ...
        with transaction.atomic():
            for row in unsynced_rows:
                ...
                try:
                    ...
                    queue_item.save(update_fields=['status', 'position', 'actual_completion_time'])
                    updated_count += 1
        ...
        orphaned = CoffeeQueue.objects.filter(
            order__isnull=True
        ) | CoffeeQueue.objects.exclude(
            order_id__in=OrderModel.objects.values('id')
        )
        ...
        self.stdout.write(f'   刪除孤立記錄: {orphaned_count if orphaned_count > 0 else 0} 條')
