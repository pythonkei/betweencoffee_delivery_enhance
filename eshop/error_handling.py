...
class ErrorHandler:
    ...
    def wrap_function(self, func, context: str = None, log_success: bool = True):
        """
    ...
    def _log_error(
        self, error_details: Dict[str, Any], log_level: str, error: Exception
    ):
        """記錄錯誤日誌"""
        log_message = (
...
def handle_success(
    operation: str, data: Optional[Dict[str, Any]] = None, message: Optional[str] = None
) -> Dict[str, Any]:
    """
    全局成功處理便捷函數
    ...
    return global_error_handler.handle_success(
        operation=operation, data=data, message=message
    )
...
def wrap_function(func, context: str = None, log_success: bool = True):
    ...
    return global_error_handler.wrap_function(
        func=func, context=context, log_success=log_success
    )
