...
def convert_6_digit_codes_to_4_digits(apps, schema_editor):
    ...
    # 获取所有6位提取码的订单
    orders_with_6_digit = OrderModel.objects.filter(pickup_code__regex=r"^.{6}$")
    ...
    for order in orders_with_6_digit:
        ...
        if len(digits) >= 4:
            ...
            if len(hash_digits) >= 4:
                ...
                # 最后的手段：生成随机4位数字
                new_code = "".join(secrets.choice(string.digits) for _ in range(4))
        ...
        # 检查新码是否已存在
        while (
            OrderModel.objects.exclude(id=order.id)
            .filter(pickup_code=new_code)
            .exists()
        ):
            # 如果冲突，生成新的随机码
            new_code = "".join(secrets.choice(string.digits) for _ in range(4))
...
class Migration(migrations.Migration):
    ...
    operations = [
        # 首先转换现有的6位提取码为4位数字
        migrations.RunPython(convert_6_digit_codes_to_4_digits, reverse_convert_codes),
        # 然后修改字段长度从6改为4
