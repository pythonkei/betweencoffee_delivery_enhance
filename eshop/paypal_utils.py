...
def get_paypal_access_token():
    ...
    try:
        ...
        if not hasattr(settings, "PAYPAL_CLIENT_ID") or not settings.PAYPAL_CLIENT_ID:
            ...
            return None
        if (
            not hasattr(settings, "PAYPAL_CLIENT_SECRET")
        ...
        response = requests.post(
            f"{base_url}/v1/oauth2/token", headers=headers, data=data, timeout=30
        )
...
def create_paypal_payment(order, request):
    ...
    try:
        ...
        if response.status_code != 201:
            logger.error(
                f"创建PayPal订单失败: {response.status_code} - {response.text}"
            )
            return None
...
def capture_paypal_payment(payment_id):
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/v2/checkout/orders/{payment_id}/capture", headers=headers
        )
        ...
        if response.status_code != 201:
            logger.error(
                f"PayPal支付捕获失败: {response.status_code} - {response.text}"
            )
            return False
