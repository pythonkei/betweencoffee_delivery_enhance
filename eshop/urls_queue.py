...
from .views.queue_views import get_unified_queue_data  # 核心：统一的队列数据API
from .views.queue_views import (  # 页面视图; staff_order_management 已移至 eshop/urls_staff.py（路徑: /eshop/staff/order-management/）; ==================== 统一数据API（新增） ====================; ==================== 操作API（保持不变） ====================; ==================== 数据修复API（保持不变） ====================; ==================== 订单详情API（保持不变） ====================; ==================== 已废弃的API（将被移除或注释掉） ====================; get_queue_updates,           # 已由 get_unified_queue_data 替代; get_badge_summary,          # 已由 get_unified_queue_data 替代; preparing_orders_count_api, # 已由 get_unified_queue_data 替代; ready_orders_count_api,     # 已由 get_unified_queue_data 替代; completed_orders_count_api, # 已由 get_unified_queue_data 替代; preparing_orders_json_api,  # 已由 get_unified_queue_data 替代; ready_orders_json_api,      # 已由 get_unified_queue_data 替代; completed_orders_json_api,  # 暂时保留（需要扩展统一API）
    cleanup_queue_data,
    force_sync_api,
    mark_as_collected,
    mark_as_ready_api,
...
urlpatterns = [
    ...
    path(
        ...
        order_details_for_waiting_api,
        name="order_details_for_waiting_api",
    ),
