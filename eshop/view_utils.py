...
from .models import BeanItem, CoffeeItem, CoffeeQueue, OrderModel
...
def handle_payment_by_order_id(request, order_id):
    ...
    try:
        if not order_id:
            return redirect(
                reverse("eshop:order_payment_confirmation") + "?payment_status=error"
            )
        ...
        if order.payment_status == "paid":
            ...
            if not result["success"]:
                ...
                return redirect(
                    ...
                    + f"?order_id={order_id}&payment_status=error"
                )
...
def send_payment_notifications(order):
    ...
    try:
        if (
            order.phone
            and hasattr(order, "user")
            and order.user
            and order.user.is_authenticated
        ):
            try:
    ...
    try:
        ...
        for item1, item2 in zip(order1_items, order2_items):
            if (
                ...
                or item1.get("id") != item2.get("id")
                or item1.get("quantity") != item2.get("quantity")
            ):
                return False
        return True
    except Exception as e:
...
def log_payment_attempt(order_id, payment_method, status, details=None):
    ...
    log_order_event(
        ...
        "payment",
        message,
        extra_data={"payment_method": payment_method, "status": status},
    )
        ...
        for item in queue_items:
            queue_info = {
                ...
                "order_number": getattr(
                    item.order, "order_number", f"#{item.order.id}"
                ),
                "position": item.position,
                "status": item.status,
                "status_display": item.get_status_display(),
                "coffee_count": getattr(item, "coffee_count", 0),
                "preparation_time_minutes": getattr(
            ...
            if include_times:
                queue_info.update(
                    {
                        "added_to_queue": (
                            item.added_to_queue.isoformat()
                            if item.added_to_queue
                            else None
                        ),
                        "started_at": (
                        ...
                        "ready_time": (
                            item.ready_time.isoformat() if item.ready_time else None
                        ),
                        "estimated_prepare_time": (
                            item.estimated_prepare_time.isoformat()
                            if item.estimated_prepare_time
                            else None
                        ),
                    }
                )
...
def get_queue_statistics():
    ...
    try:
        ...
        completed_queues = CoffeeQueue.objects.filter(
            status="completed", ready_time__isnull=False, started_at__isnull=False
        ...
        if completed_queues:
            ...
            for queue in completed_queues:
                if queue.ready_time and queue.started_at:
                    total_seconds += (
                        queue.ready_time - queue.started_at
                    ).total_seconds()
        ...
        return {
            ...
            "avg_preparation_minutes": (
                round(avg_preparation_time / 60, 1) if avg_preparation_time else 0
            ),
    ...
    try:
        ...
        if status == "paid":
            result = OrderStatusManager.process_payment_success(
                order_id=order.id, request=None
            )
            if not result["success"]:
            ...
            if order.status == "pending":
                ...
                if result["success"]:
                    return {
                        ...
                        "reason": "订单已超时，已自动取消",
                        "needs_redirect": True,
                        "redirect_url": f"/eshop/order/{order.id}/",
                    }
                else:
                    return {
                        "can_pay": False,
