...
class PerformanceOptimizer:
        ...
        try:
            ...
            if not recent_data:
                return {
                    ...
                    "data": {"count": 0, "average": 0, "min": 0, "max": 0, "latest": 0},
                }
    ...
    def monitor_allocation_performance(self):
        ...
        try:
            ...
            self.record_metric(
                "system.active_baristas", system_status["active_baristas"]
            )
            ...
            self.record_metric(
                "system.available_capacity", system_status["available_capacity"]
            )
            self.record_metric(
                "system.utilization_rate", system_status["utilization_rate"]
            )
            ...
            return {"success": False, "message": f"監控失敗: {str(e)}", "data": None}
    ...
    def optimize_allocation_strategy(self):
        ...
        try:
            ...
            return {"success": False, "message": f"優化失敗: {str(e)}", "data": None}
        ...
        try:
            ...
            report = {
                ...
                "data": {
                    ...
                    "optimization_suggestions": optimization_result.get("data", {}).get(
                        "optimization_suggestions", []
                    ),
                    ...
                    "total_optimization_suggestions": (
                        len(
                            optimization_result.get("data", {}).get(
                                "optimization_suggestions", []
                            )
                        )
                        + len(
                            database_result.get("data", {}).get(
                                "optimization_suggestions", []
                            )
                        )
                        + len(
                            cache_result.get("data", {}).get(
                                "optimization_suggestions", []
                            )
                        )
                    ),
            ...
            self.logger.info(
                f"性能報告生成完成: {report['data']['total_optimization_suggestions']} 個建議"
            )
    ...
    def clear_old_metrics(self, days_old=7):
        ...
        try:
            ...
            for metric_name, records in list(self.metrics.items()):
                # 過濾掉舊數據
                new_records = [r for r in records if r["timestamp"] >= cutoff_time]
            ...
            return {"success": False, "message": f"清理失敗: {str(e)}", "data": None}
