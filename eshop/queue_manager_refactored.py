...
class CoffeeQueueManager:
    ...
    def add_order_to_queue(self, order, use_priority=True):
        ...
        try:
            ...
            self.logger.info(f"☕ 訂單 #{order.id} 咖啡杯數計算: {coffee_count} 杯")
            ...
            # 計算製作時間
            preparation_time = unified_time_service.calculate_preparation_time(
                coffee_count
            )
            self.logger.info(
            ...
            if use_priority:
                ...
                if reordered:
                    self.logger.info(f"🔄 訂單 #{order.id} 隊列重新排序完成")

            # 更新隊列時間
            time_updated = self.update_estimated_times()
            if time_updated:
                self.logger.info(f"⏰ 訂單 #{order.id} 隊列時間更新完成")
    ...
    def start_preparation(self, queue_item, barista_name=None):
        ...
        try:
            ...
            time_updated = self.update_estimated_times()
            if time_updated:
                self.logger.info(f"⏰ 訂單 #{queue_item.order.id} 隊列時間更新完成")
    ...
    def mark_as_ready(self, queue_item, staff_name=None):
        ...
        try:
            ...
            if order.status == "ready":
                self.logger.info(f"ℹ️ 訂單 #{order.id} 已經是就緒狀態，無需再次標記")
            ...
            queue_item.actual_completion_time = (
                unified_time_service.get_hong_kong_time()
            )

            if not queue_item.actual_start_time:
                queue_item.actual_start_time = (
                    queue_item.actual_completion_time
                    - timedelta(minutes=queue_item.preparation_time_minutes)
                )
                self.logger.info(
            ...
            result = OrderStatusManager.mark_as_ready_manually(
                order_id=order.id, staff_name=staff_name or "queue_manager"
            )
            ...
            if not result.get("success"):
                ...
                return handle_error(
                    error=Exception(
                        f"OrderStatusManager標記失敗: {result.get('message')}"
                    ),
            ...
            if not order.ready_at:
                ...
                self.logger.info(f"⏰ 訂單 #{order.id} 同步就緒時間: {order.ready_at}")
    ...
    def calculate_preparation_time(self, coffee_count):
        ...
        """
        try:
            preparation_minutes = unified_time_service.calculate_preparation_time(
                coffee_count
            )
            self.logger.debug(
                f"計算製作時間: {coffee_count} 杯 -> {preparation_minutes} 分鐘"
            )
    ...
    def _calculate_position(self, order, coffee_count, use_priority):
        ...
        try:
            ...
            self.logger.debug(
                f"訂單 #{order.id} 位置計算: {position} (優先級: {use_priority})"
            )
            return position
    ...
    def _get_next_simple_position(self):
        ...
        try:
            last_item = (
                CoffeeQueue.objects.filter(status="waiting")
                .order_by("-added_at")
                .first()
            )
    ...
    def _calculate_priority_position(self, order):
        ...
        """
        try:
            waiting_queues = CoffeeQueue.objects.filter(status="waiting").order_by(
                "added_at"
            )
            ...
            if order.order_type == "quick":
                for queue in waiting_queues:
                    if queue.order.order_type != "quick":
                        self.logger.debug(
                            f"訂單 #{order.id} 優先級位置: {queue.position} (插入到普通訂單前)"
                        )
                ...
                self.logger.debug(
                    f"訂單 #{order.id} 優先級位置: {position} (添加到隊列末尾)"
                )
                return position
            ...
            # 普通訂單處理
                ...
                position = (
                    last_quick_position + 1
                    if last_quick_position > 0
                    else len(waiting_queues) + 1
                )
                self.logger.debug(
                    f"訂單 #{order.id} 優先級位置: {position} (添加到快速訂單後)"
                )
                return position
    ...
    def _check_and_reorder_queue(self):
        ...
        try:
            ...
            # 排序：快速訂單優先，然後按加入隊列時間
            queues_info.sort(
                key=lambda x: (0 if x["order_type"] == "quick" else 1, x["added_at"])
            )
    ...
    def recalculate_all_order_times(self):
        ...
        try:
            ...
            for order in quick_orders:
                try:
                    if (
                        ...
                        if time_info:
                            order.estimated_ready_time = time_info[
            ...
            for order in quick_orders:
                try:
                    if (
                        ...
                        and order.should_be_in_queue_by_now()
                    ):
            ...
            if integrity_check_result.get("success"):
                ...
                self.logger.warning(
                    f"⚠️ 隊列完整性檢查失敗: {integrity_check_result.get('message')}"
                )
                has_issues = True
    ...
    def verify_queue_integrity(self):
        ...
        try:
            ...
            # 檢查ready訂單位置
            ready_with_position = CoffeeQueue.objects.filter(
                status="ready", position__gt=0
            )
            if ready_with_position.exists():
                issues.append(
                    f"發現 {ready_with_position.count()} 個ready訂單有隊列位置"
                )
            ...
            for i, queue in enumerate(waiting_queues):
                if i > 0:
                    ...
                    if (
                        queue.added_at
                        and prev.added_at
                        and queue.added_at < prev.added_at
                    ):
                        issues.append(
                            f"訂單 #{queue.order.id} added_at 順序異常: {queue.added_at} < {prev.added_at}"
                        )
            ...
            if has_issues:
                self.logger.warning(f"⚠️ 隊列完整性檢查發現問題: {len(issues)} 個問題")
                for issue in issues:
    ...
    def fix_queue_positions(self):
        ...
        try:
            # 重置ready訂單位置
            ready_positions_reset = CoffeeQueue.objects.filter(
                status="ready", position__gt=0
            ).update(position=0)
    ...
    def add_order_to_queue_with_smart_allocation(self, order, use_priority=True):
        ...
        try:
            ...
            # 如果有推薦的員工，更新隊列項
            if allocation_result.get("success") and allocation_result.get(
                "recommended_barista_name"
            ):
                ...
                if recommendations.get("success") and recommendations.get(
                    ...
                    for rec in recommendations["recommendations"]:
                        if rec["type"] == "allocation" and rec.get("barista_name"):
                            ...
                            self.logger.info(f"✅ 智能分配建議: 使用 {barista_name}")
                            break
    ...
    def get_smart_recommendations(self, order_id):
        ...
        try:
            ...
            data = {
                ...
                "total_recommendations": recommendations_result.get(
    ...
    def optimize_queue_with_smart_allocation(self):
        ...
        try:
            ...
            if len(barista_details) >= 2:
                # 檢查兩個員工的負載差異
                load_diff = abs(
                    barista_details[0]["current_load"]
                    - barista_details[1]["current_load"]
                )
