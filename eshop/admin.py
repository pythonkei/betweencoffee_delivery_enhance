...
class CoffeeItemAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "氣泡位置（6 選 1；動畫不變）",
            {
                "fields": ("bubble_pos_1", "bubble_pos_2", "bubble_pos_3"),
                "description": "三顆氣泡可各自選一個落點：左上／左中間／左下／右上／右中間／右下。6 個落點都自動貼在照片去背內容的外緣＝咖啡杯外圍，水平位置會依該顆氣泡的圖形寬度與照片幾何自動縮放（所有斷點通用），換照片後自動重算。預設＝左中間／右下／右上（原本的設計位置）；彈出動畫效果完全不受影響。",
            },
        ),
        (
            "氣泡定位（留空=存檔時自動量測照片）",
