    ...
    fieldsets = (
        ...
        (
            "詳情頁氣泡文字（留空=不顯示）",
            {
                ...
                "description": "詳情頁照片上輪播的店員氣泡文字（每 3 秒輪替一顆）。留空 = 該顆不顯示；三顆全空則整組不出現。建議字數：1 → 5~7 字、2 → 5~7 字、3 → 5~6 字（氣泡圖形為固定尺寸，字數過多會超出圖形）。",
            },
        ),
        (
            "氣泡位置（6 選 1；動畫不變）",
            {
                ...
                "description": "三顆氣泡可各自選一個落點：左上／左中間／左下／右上／右中間／右下。6 個落點都自動貼在照片去背內容的外緣＝咖啡杯外圍，水平位置會依該顆氣泡的圖形寬度與照片幾何自動縮放（所有斷點通用）。預設＝左中間／右下／右上（原本的設計位置）；彈出動畫效果完全不受影響。※ 氣泡定位所需的照片幾何（內容左右緣、寬高比、尺寸倍率）由系統在存檔時自動量測，換照片後會自動重算，不需人工設定。",
            },
        ),
        (
            "Sticker 貼紙（留空=不顯示）",
