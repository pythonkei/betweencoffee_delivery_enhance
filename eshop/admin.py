...
def staff_order_management(request):
    ...
    preparing_orders = OrderModel.objects.filter(
        ...
        "-is_quick_order", "created_at"
    )  # 快速訂單優先

    ready_orders = OrderModel.objects.filter(
        payment_status="paid", status="ready"
    ).order_by(
        "-is_quick_order", "created_at"
    )  # 快速訂單優先
...
def mark_order_ready(request, order_id):
    ...
    try:
        ...
        result = OrderStatusManager.mark_as_ready_manually(
            order_id=order_id, staff_name=staff_name
        )
    ...
    for order in queryset:
        if order.payment_status == "paid" and order.status == "pending":
            result = OrderStatusManager.mark_as_preparing_manually(
                order_id=order.id, staff_name=staff_name
            )
            if result["success"]:
                success_count += 1
            else:
...
class OrderModelAdmin(admin.ModelAdmin):
    ...
    def colored_quick_order(self, obj):
        ...
        return format_html('<span style="color: {}; font-size: 1.5em;">⬤</span>', color)
    ...
    def payment_status_with_badge(self, obj):
        ...
        color, text = status_map.get(
            obj.payment_status, ("secondary", obj.payment_status)
        )
    ...
    def display_items(self, obj):
        ...
        for item in items:
            ...
            html += (
                f'<p style="margin: 0; color: #666;">價格: ${item["total_price"]}</p>'
            )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            path(
                "staff-management/",
                self.admin_site.admin_view(staff_order_management),
                name="staff_order_management",
            ),
            path(
                "mark-ready/<int:order_id>/",
                self.admin_site.admin_view(mark_order_ready),
                name="mark_order_ready",
            ),
            path(
                "mark-completed/<int:order_id>/",
                self.admin_site.admin_view(mark_order_completed),
...
class CoffeeItemAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "詳情頁氣泡文字（留空=不顯示）",
            {
                ...
                "description": "詳情頁照片上輪播的店員氣泡文字（每 3 秒輪替一顆）。留空 = 該顆不顯示；三顆全空則整組不出現。建議字數：1 → 5~7 字、2 → 5~7 字、3 → 5~6 字（氣泡圖形為固定尺寸，字數過多會超出圖形）。氣泡會自動排在照片「不透明內容」之外，照片幾何見下方欄位組。",
            },
        ),
        (
            "氣泡落點微調（留空=預設位置；逐款咖啡可用）",
            {
                "fields": (
                    "bubble_dx_1", "bubble_dy_1",
                    "bubble_dx_2", "bubble_dy_2",
                    "bubble_dx_3", "bubble_dy_3",
                ),
                "description": "三顆氣泡的落點可用偏移量逐款微調，單位是「照片寬／高的百分比」（正值＝往右／往下，負值＝往左／往上）。留空＝維持預設位置（左側／右下／右上，都自動貼在照片去背內容外緣＝咖啡杯外圍，換照片後會自動重算）。例：這款的氣泡 1 想更高 → 垂直偏移填 -6（往上 6% 照片高）。氣泡動畫（彈出）效果不受影響。",
            },
        ),
        (
            "氣泡定位（留空=存檔時自動量測照片）",
            {
                ...
                "description": "三顆氣泡都排在照片「不透明內容」之外，故需要照片實際內容的左右緣（占照片寬 %）與寬高比；橫式照片的氣泡尺寸倍率會自動設為 1/寬高比，避免氣泡相對照片過大。一般不需手動改：換圖後把這四欄清空再存一次，即會用 Pillow 重新量測去背 PNG 的不透明範圍。",
            },
        ),
        (
            "Sticker 貼紙（留空=不顯示）",
    ...
    def index_image_preview(self, obj):
        if obj.image_index:
            ...
            return format_html(
                '<img src="{}" style="width: 50px; height: 50px; object-fit: cover; opacity: 0.5;" title="使用详情页图片" />',
                obj.image.url,
            )
        return "-"
