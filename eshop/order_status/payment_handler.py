    ...
    def process_payment_success(cls, order_id, request=None):
        ...
        try:
            ...
            if order.payment_status not in ['paid', 'payment_pending']:
                ...
                order.payment_status = 'paid'
                order.paid_at = timezone.now()
                logger.info(f"✅ FPS 訂單 #{order_id} 員工確認付款，支付狀態更新為 paid")
            else:
            ...
            if order.status == 'pending':
                ...
                if order_type['is_beans_only']:
                    ...
                    order.status = 'waiting'
                logger.info(f"✅ FPS/現金訂單 #{order_id} 支付狀態從 payment_pending 更新為 paid，訂單狀態更新為 {order.status}")

            # ✅ 修復：保存所有更新
            order.save()
            logger.info(f"✅ 訂單 #{order_id} 保存成功: status={order.status}, payment_status={order.payment_status}, paid_at={order.paid_at}")
            ...
            if manager.should_add_to_queue():
                ...
                if queue_result.get('success'):
                    ...
                    logger.error(f"訂單 {order.id} 加入隊列失敗: {queue_result.get('message')}")
            else:
            ...
            if order.user:
                try:
                    ...
                    logger.info(f"✅ 用戶 {order.user.username} 訂單 #{order.id} 獲得 {points_earned} 積分")
            ...
            return {'success': False, 'message': '訂單不存在', 'error': 'Order not found'}
        except Exception as error:
            logger.error(f"❌ 處理支付成功失敗: {str(error)}", exc_info=True)
            return {'success': False, 'message': f'處理失敗: {str(error)}', 'error': str(error)}
