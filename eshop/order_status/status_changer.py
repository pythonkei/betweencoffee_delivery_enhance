...
class StatusChanger:
    ...
    def mark_as_preparing_manually(cls, order_id, barista_name=None, preparation_minutes=None):
        ...
        try:
            ...
            order.estimated_ready_time = hk_time + timedelta(minutes=preparation_minutes)
            ...
            if queue_item:
                ...
                queue_item.estimated_completion_time = hk_time + timedelta(minutes=preparation_minutes)
                if barista_name:
            ...
            # 6. 記錄日誌
            logger.info(f"✅ 訂單 #{order_id} 已開始製作，操作員: {barista_name or 'system'}")
            ...
            logger.error(f"❌ 標記訂單 {order_id} 為製作中失敗: {str(e)}", exc_info=True)
            return {'success': False, 'message': str(e)}
    ...
    def mark_as_ready_manually(cls, order_id, staff_name=None):
        ...
        try:
            ...
            if queue_item:
                ...
                logger.info(f"✅ 訂單 #{order_id} 隊列項已更新: 狀態 → ready, 位置 {old_position} → 0")
    ...
    def mark_as_completed_manually(cls, order_id, staff_name=None):
        ...
        try:
            ...
            if result.get('success'):
                ...
                logger.error(f"❌ 標記訂單 #{order_id} 為已提取失敗: {result.get('error')}")
