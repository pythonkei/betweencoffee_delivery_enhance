...
class StatusDisplay:
    ...
    def _get_coffee_order_status(self):
        ...
        status_info = {
            ...
            'show_progress_bar': self.order.payment_status == 'paid' and not OrderTypeAnalyzer.analyze_order_type(self.order)['is_beans_only'],
    ...
    def _calculate_progress(self):
        ...
        try:
            if self.order.status == 'ready':
                ...
                # 根據已製作時間計算進度
                if self.order.preparation_started_at and self.order.estimated_ready_time:
                    now = timezone.now()
                    total_duration = (self.order.estimated_ready_time - self.order.preparation_started_at).total_seconds()
                    elapsed = (now - self.order.preparation_started_at).total_seconds()

                    if total_duration > 0:
                        percentage = min(int((elapsed / total_duration) * 100), 99)
                        return {'percentage': percentage, 'display': f'{percentage}% 完成'}
