...
class BaristaWorkloadManager:
    ...
    def get_barista_workload(self, barista_id):
        ...
        try:
            ...
            current_queues = CoffeeQueue.objects.filter(
                barista=barista.name, status="preparing"
            )
    ...
    def complete_order_for_barista(self, order_id, barista_id):
        ...
        workload = self.get_barista_workload(barista_id)
        if workload:
            self.logger.info(f"✅ 員工 #{barista_id} 完成訂單 #{order_id}，釋放容量")
            return True
        return False
...
class SmartOrderAllocator:
    ...
    def allocate_order(self, order, strategy="balanced"):
        ...
        try:
            ...
            if strategy == "balanced":
                ...
                recommended = self._round_robin_allocation(
                    workloads, coffee_count, order
                )
            else:
                recommended = self._balanced_allocation(workloads, coffee_count, order)
            ...
            estimated_time = self._calculate_estimated_time(
                coffee_count, recommended["efficiency_factor"] if recommended else 1.0
            )
    ...
    def _balanced_allocation(self, workloads, coffee_count, order):
        ...
        # 過濾可用員工（有足夠容量）
        available_baristas = [
            w for w in workloads if w["available_slots"] >= coffee_count
        ]
    ...
    def _split_allocation(self, workloads, coffee_count, order):
        ...
        sorted_workloads = sorted(
            workloads, key=lambda x: x["available_slots"], reverse=True
        )
...
class DynamicTimeCalculator:
    ...
    def calculate_order_preparation_time(
        self, order, barista_efficiency=1.0, is_quick_order=False
    ):
        """
        計算訂單的預計製作時間
    ...
    def calculate_total_estimated_time(
        self, order, barista_efficiency=1.0, queue_position=0
    ):
        """
        計算總預計時間（等待時間 + 製作時間）
    ...
    try:
        ...
        if result["success"] and not result.get("skip_allocation"):
            ...
            workload_manager.assign_order_to_barista(
                order, result["recommended_barista_id"], result["coffee_count"]
            )
...
def get_recommendations_for_order(order_id):
    ...
    try:
        ...
        time_estimate = time_calculator.calculate_total_estimated_time(
            order, barista_efficiency=1.0, queue_position=queue_position
        )
