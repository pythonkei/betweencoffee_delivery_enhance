...
class QueryOptimizer:
    ...
    def get_preparing_queues():
        ...
        """
        return (
            CoffeeQueue.objects.filter(status="preparing")
            .select_related("order")
    ...
    def get_coffee_queue_map(order_ids: List[int]) -> Dict[int, CoffeeQueue]:
        ...
        coffee_queues = CoffeeQueue.objects.filter(order_id__in=order_ids).order_by(
            "order_id", "-id"
        )  # 按訂單ID和ID降序排序
    ...
    def get_orders_with_queue_info(order_ids: List[int]):
        ...
        return OrderModel.objects.filter(id__in=order_ids).prefetch_related(
            Prefetch(
                "coffeequeue_set",
...
class BatchOrderProcessor:
    ...
    def prepare_ready_orders_batch(orders, now=None, hk_tz=None):
        ...
        for order in orders:
            ...
            is_beans_only = (
                all(item.get("type") == "bean" for item in items) if items else False
            )
            ...
            if is_beans_only:
                ...
                order_data["barista"] = (
                    queue_item.barista
                    if queue_item and queue_item.barista
                    else "未分配"
                )
