...
def barista_workload_api(request):
    ...
    try:
        ...
        recent_order = OrderModel.objects.filter(
            payment_status="paid", status__in=["waiting", "preparing"]
        ).first()
        ...
        # 合併數據
        response_data = {**system_status, "recommendations": recommendations}

        logger.info(
            f"✅ 工作負載數據獲取成功: {len(system_status.get('barista_details', []))} 個員工"
        )

        return JsonResponse(
            {"success": True, "message": "工作負載數據獲取成功", "data": response_data}
        )
        ...
        return JsonResponse(
            {"success": False, "message": f"獲取工作負載失敗: {str(e)}", "data": None},
            status=500,
        )
...
def optimize_queue_api(request):
    ...
    try:
        ...
        if result.get("success"):
            data = result["data"]
            logger.info(
                f"✅ 隊列優化成功: 優化 {data.get('orders_optimized', 0)} 個訂單"
            )

            return JsonResponse(
                {"success": True, "message": "隊列優化成功", "data": data}
            )
        else:
        ...
        return JsonResponse(
            {"success": False, "message": f"隊列優化失敗: {str(e)}", "data": None},
            status=500,
        )
...
def order_recommendations_api(request, order_id):
    ...
    try:
        ...
        if recommendations_result.get("success"):
            ...
            logger.info(
                f"✅ 訂單 #{order_id} 建議獲取成功: {len(data.get('recommendations', []))} 個建議"
            )

            return JsonResponse(
                {"success": True, "message": "建議獲取成功", "data": data}
            )
        else:
            logger.error(
                f"❌ 訂單 #{order_id} 建議獲取失敗: {recommendations_result.get('message')}"
            )
        ...
        return JsonResponse(
            {"success": False, "message": f"建議獲取失敗: {str(e)}", "data": None},
            status=500,
        )
...
def system_status_api(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": True, "message": "系統狀態獲取成功", "data": response_data}
        )
        ...
        return JsonResponse(
            {"success": False, "message": f"系統狀態獲取失敗: {str(e)}", "data": None},
            status=500,
        )
...
def assign_order_api(request, order_id):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": f"訂單 #{order_id} 不存在", "data": None},
                status=404,
            )
        ...
        if barista_id:
            ...
            try:
                ...
                if assigned:
                    ...
                    return JsonResponse(
                        {"success": False, "message": "訂單分配失敗", "data": None},
                        status=400,
                    )
        ...
        if allocation_result.get("success"):
            ...
            if data.get("recommended_barista_id"):
                ...
                workload_manager.assign_order_to_barista(
                    order, data["recommended_barista_id"], coffee_count
                )

            return JsonResponse(
                {"success": True, "message": "訂單分配成功", "data": allocation_result}
            )
        else:
            logger.error(
                f"❌ 訂單 #{order_id} 分配失敗: {allocation_result.get('message')}"
            )
        ...
        return JsonResponse(
            {"success": False, "message": f"訂單分配失敗: {str(e)}", "data": None},
            status=500,
        )
...
def update_barista_status_api(request, barista_id):
    ...
    try:
        ...
        if not request.body:
            return JsonResponse(
                {"success": False, "message": "請求體為空", "data": None}, status=400
            )
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": "請求體JSON解析失敗", "data": None},
                status=400,
            )
        ...
        return JsonResponse(
            {"success": False, "message": f"員工狀態更新失敗: {str(e)}", "data": None},
            status=500,
        )
    ...
    try:
        ...
        if count > 0:
            average_preparation_time = (
                total_time.total_seconds() / count / 60
            )  # 轉換為分鐘

        # 計算每小時訂單數
        one_hour_ago = timezone.now() - timedelta(hours=1)
        recent_orders = OrderModel.objects.filter(
            created_at__gte=one_hour_ago, payment_status="paid"
        ).count()
