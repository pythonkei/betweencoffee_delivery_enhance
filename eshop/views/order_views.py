...
class OrderConfirm(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.info(
                f"訂單價格計算: 原始總價={original_total_price}, 折扣後總價={final_total_price}, 總折扣={discount_data['total_discount']}"
            )
            ...
            if not existing_order:
                try:
                    ...
                    logger.info(
                        f"訂單折扣信息: 優惠券={discount_data['applied_coupon_code']}, 優惠券折扣={discount_data['coupon_discount']}, 會員折扣率={discount_data['loyalty_discount_rate']}, 會員折扣金額={discount_data['loyalty_discount_amount']}"
                    )
                        ...
                        try:
                            ...
                            for reward in unused_rewards:
                                if (
                                    ...
                                    logger.info(
                                        f"獎勵 {reward.reward_name} 已標記為在訂單 #{order.id} 中使用"
                                    )
                        except Exception as e:
...
def quick_order(request):
    ...
    logger.info(
        f"快速订单数据已保存到session, pickup_time: {pickup_time_display}, choice: {pickup_time_choice}"
    )
...
def order_payment_confirmation(request, order_id=None):
    ...
    try:
        ...
        logger.info(
            f"訂單 {order.id} 當前狀態: status={order.status}, "
        ...
        if (
            ...
            try:
                ...
                logger.info(
                    f"用戶 {request.user.username} 訂單 {order.id} "
