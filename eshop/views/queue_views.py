...
from eshop.queue_manager_refactored import (
    CoffeeQueueManager,
...
def start_preparation_api(request, order_id):
    ...
    try:
        ...
        if not result["success"]:
            return JsonResponse(
                {"success": False, "message": result["message"]}, status=400
            )

        order = result["order"]
        items = order.get_items()
        coffee_count = sum(
            item.get("quantity", 1) for item in items if item.get("type") == "coffee"
        )
        ...
        return JsonResponse(
            {"success": False, "message": f"系統錯誤: {str(e)}"}, status=500
        )
...
def recalculate_all_times_api(request):
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": str(e), "message": "系統錯誤"}, status=500
        )
...
def mark_as_ready_api(request, order_id):
    ...
    try:
        ...
        if result.get("success"):
            logger.info(
                f"✅ 舊API: 訂單 #{order_id} 已通過OrderStatusManager標記為就緒"
            )
            ...
            logger.error(
                f"❌ 舊API: 標記訂單 #{order_id} 為就緒失敗: {result.get('error')}"
            )
            return JsonResponse(
                {
                    "success": False,
        ...
        return JsonResponse(
            {"success": False, "error": f"伺服器錯誤: {str(e)}"}, status=500
        )
...
def mark_as_collected(request, order_id):
    ...
    try:
        ...
        if result.get("success"):
            logger.info(
                f"✅ 舊API: 訂單 #{order_id} 已通過OrderStatusManager標記為已提取"
            )
            ...
            logger.error(
                f"❌ 舊API: 標記訂單 #{order_id} 為已提取失敗: {result.get('error')}"
            )
            return JsonResponse(
                {
                    "success": False,
...
def force_sync_api(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "message": "强制同步完成"})
...
def order_details_for_waiting_api(request, order_id):
    ...
    try:
        ...
        if not request.user.is_staff:
            return JsonResponse(
                {"success": False, "error": "无权访问此订单详细信息"}, status=403
            )
    ...
    try:
        ...
        for order in orders:
            ...
            if result["success"]:
                ...
                error_count += 1
                logger.error(
                    f"重置訂單 {order.id} 狀態為等待中失敗: {result['message']}"
                )
