...
class TimeCalculators:
    ...
    @staticmethod
    def calculate_latest_start_time(
        target_time, preparation_minutes, buffer_minutes=None
    ):
        """
        計算最晚開始時間
    ...
    def calculate_quick_order_times(pickup_choice, coffee_count, current_time):
        ...
        try:
            ...
            # 計算製作時間
            preparation_minutes = TimeCalculators.calculate_preparation_time(
                coffee_count
            )
            ...
            logger.info(
                f"快速訂單時間計算: 取貨{minutes_to_add}分鐘後, {coffee_count}杯"
            )
            return result
    ...
    def calculate_progress_percentage(
        start_time, estimated_completion_time, current_time
    ):
        """
        計算製作進度百分比
