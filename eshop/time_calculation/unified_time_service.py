...
class UnifiedTimeService:
    ...
    def calculate_quick_order_times(self, order):
        ...
        try:
            ...
            latest_start_time = estimated_pickup_time - timedelta(
            ...
            logger.info(
                f"快速訂單時間計算: 訂單#{order.id}, 取貨{minutes_to_add}分鐘後"
            )
            return result
            ...
            # 返回默認值
            return {
                "estimated_pickup_time": self.get_hong_kong_time()
            ...
            if order.order_type == "quick" and has_coffee:
                if order.pickup_time_choice:
                    ...
                    return {
                        ...
                        "icon": (
                            "fa-clock" if not is_urgent else "fa-exclamation-triangle"
                        ),
                        "is_immediate": False,
                        "is_urgent": is_urgent,
