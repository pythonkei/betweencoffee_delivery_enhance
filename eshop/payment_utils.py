...
def get_payment_tools(payment_method):
    ...
    tools = {
        ...
        "cash": {"process": lambda order, request: True},
    }
...
def check_alipay_keys():
    ...
    try:
        ...
        if not hasattr(settings, "ALIPAY_APP_PRIVATE_KEY"):
            return {"success": False, "error": "ALIPAY_APP_PRIVATE_KEY 未配置"}
        ...
        try:
            ...
            return {"success": False, "error": f"支付宝客户端创建失败: {str(e)}"}
...
def validate_fps_payment(reference, amount):
    """验证FPS支付（模拟）"""
    return {"success": True, "reference": reference, "amount": amount, "verified": True}
...
def validate_payment_amount(order, payment_amount):
    ...
    try:
        ...
        if abs(order_amount - payment_amount) <= tolerance:
            ...
            logger.warning(
                f"支付金额不匹配: 订单金额={order_amount}, 支付金额={payment_amount}"
            )
            return False
    ...
    try:
        ...
        if payment_type == "alipay":
            ...
            if not payment_id or not payer_id:
                return {"success": False, "error": "缺少支付信息"}

            is_captured = capture_paypal_payment(payment_id)
            if not is_captured:
                return {"success": False, "error": "支付捕获失败"}
            ...
            return {
                ...
                "payment_data": {"payment_id": payment_id, "payer_id": payer_id},
            }

        else:
            return {"success": False, "error": f"不支持的支付类型: {payment_type}"}
...
def get_available_payment_methods():
    ...
    if is_payment_method_available("alipay"):
        methods.append(
            {"id": "alipay", "name": "支付宝", "description": "使用支付宝扫码支付"}
        )

    if is_payment_method_available("paypal"):
        methods.append(
            {"id": "paypal", "name": "PayPal", "description": "使用PayPal国际支付"}
        )

    if is_payment_method_available("fps"):
        methods.append(
            {"id": "fps", "name": "FPS转数快", "description": "香港快速支付系统"}
        )

    if is_payment_method_available("cash"):
        methods.append(
            {"id": "cash", "name": "现金支付", "description": "到店现金支付"}
        )
