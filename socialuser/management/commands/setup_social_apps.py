    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            is_render = (
                ...
                or os.environ.get("IS_RENDER") is not None
            )
            if is_railway:
                domain = os.environ.get(
                    "RAILWAY_PUBLIC_DOMAIN", "web-production-6a798.up.railway.app"
                )
                ...
                domain = os.environ.get(
                    ...
                    "betweencoffee-delivery-enhance-v1.onrender.com",
                )
            ...
            if current_site.domain != domain or current_site.name != name:
                ...
                current_site.save()
                self.stdout.write(self.style.SUCCESS(f"Site updated to: {domain}"))
            else:
                self.stdout.write(
        ...
        if google_client_id and google_secret:
            ...
            if google_apps.count() > 1:
                self.stdout.write(
                    self.style.WARNING(
                        f"Found {google_apps.count()} Google apps, cleaning duplicates..."
                    )
                )
                # 保留第一个，删除其他
            ...
            self.stdout.write(self.style.WARNING("Google OAuth credentials not set"))
        ...
        if facebook_client_id and facebook_secret:
            ...
            if facebook_apps.count() > 1:
                self.stdout.write(
                    self.style.WARNING(
                        f"Found {facebook_apps.count()} Facebook apps, cleaning duplicates..."
                    )
                )
                # 保留第一个，删除其他
            ...
            self.stdout.write(self.style.WARNING("Facebook OAuth credentials not set"))
