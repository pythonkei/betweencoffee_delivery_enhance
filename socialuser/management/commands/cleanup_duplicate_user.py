...
class Command(BaseCommand):
    ...
    def _cleanup_old_user(self):
        ...
        try:
            ...
            self.stdout.write(f"找到舊用戶: id={user.id}, username={user.username}, email={user.email}")
            ...
            self.stdout.write(self.style.SUCCESS(f"成功刪除舊用戶 {username} ({email})"))
        ...
        for user in User.objects.all():
            if not hasattr(user, 'profile'):
                try:
                    ...
                    self.stdout.write(f"  已為 {user.username} (id={user.id}) 建立 Profile")
                except Exception as e:
                    self.stderr.write(f"  為 {user.username} (id={user.id}) 建立 Profile 失敗: {e}")
        
        if users_without_profile:
            self.stdout.write(
                self.style.SUCCESS(f"已修復 {len(users_without_profile)} 個缺少 Profile 的用戶")
            )
        else:
