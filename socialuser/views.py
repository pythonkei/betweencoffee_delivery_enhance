...
from .forms import AvatarForm, EmailForm, PhoneForm, ProfileForm, UsernameForm
    ...
    context = {
        ...
        "environment_vars": {
            "OAUTH_GOOGLE_CLIENT_ID": (
                "Set" if settings.SOCIALACCOUNT_PROVIDERS.get("google") else "Not Set"
            ),
            "OAUTH_GOOGLE_SECRET": (
                "Set" if settings.SOCIALACCOUNT_PROVIDERS.get("google") else "Not Set"
            ),
            "OAUTH_FACEBOOK_CLIENT_ID": (
                "Set" if settings.SOCIALACCOUNT_PROVIDERS.get("facebook") else "Not Set"
            ),
...
def profile_view(request, username=None):
    ...
    from .forms import AvatarForm, EmailForm, PhoneForm, UsernameForm
...
def profile_emailchange(request):
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        if request.method == "POST":
            ...
            return JsonResponse(
                {
                    "success": False,
                    "message": "; ".join(errors) if errors else "請輸入有效的電子郵件",
                }
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            send_email_confirmation(request, request.user)
            messages.success(
                request, "Email updated successfully. Verification email sent."
            )
            return redirect("socialuser:profile")
...
def profile_emailverify(request):
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        ...
        try:
            ...
            if not user.email:
                return JsonResponse(
                    {"success": False, "message": "請先設定電子郵件地址"}
                )
            ...
            if not email_address.verified:
                try:
                    ...
                    return JsonResponse(
                        {"success": True, "message": f"驗證郵件已發送至 {user.email}"}
                    )
                except Exception as send_err:
                    error_msg = str(send_err)
                    logger.error(
                        f"Failed to send verification email to {user.email}: {error_msg}",
                        exc_info=True,
                    )
                    ...
                    if (
                        ...
                        or "Connection unexpectedly closed" in error_msg
                    ):
                        user_msg = "無法連接到郵件伺服器，請稍後再試"
                    elif (
                        "Authentication" in error_msg
                        or "Username and Password" in error_msg
                    ):
    ...
    try:
        ...
        logger.info(
            f"EmailAddress record: {email_address.email}, verified={email_address.verified}, created={created}"
        )
        ...
        if not email_address.verified:
            ...
            try:
                ...
                logger.error(
                    f"Failed to send confirmation email: {send_err}", exc_info=True
                )
...
def profile_phonechange(request):
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        if request.method == "POST":
            ...
            return JsonResponse(
                {"success": False, "message": "無效的電話號碼格式，請輸入8位數字"}
            )
        return JsonResponse({"success": False, "message": "僅支援 POST 請求"})
...
def profile_avatar_ajax(request):
    ...
    if (
        ...
        return JsonResponse(
            {"success": False, "message": "頭像上傳失敗，請確認檔案格式正確"}
        )
...
def profile_info_ajax(request):
    ...
    if (
        ...
        profile.save()
        return JsonResponse(
            {"success": True, "message": "個人簡介更新成功", "info": info}
        )
        ...
        for order in orders:
            ...
            order_data = {
                ...
                "original_total_price": (
                    float(order.original_total_price)
                    if order.original_total_price
                    else float(order.total_price)
                ),
                "coupon_discount": float(order.coupon_discount),
                "reward_discount_amount": float(order.reward_discount_amount),
                "applied_coupon_code": order.applied_coupon_code or "",
...
def test_email_view(request):
    ...
    try:
        ...
        html_content = render_to_string(
            "socialuser/email/email_confirmation.html", context
        )
        text_content = render_to_string(
            "socialuser/email/email_confirmation.txt", context
        )
