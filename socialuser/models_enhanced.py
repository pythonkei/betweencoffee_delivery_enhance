...
class CustomerLoyalty(models.Model):
    ...
    total_spent = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="累計消費"
    )
    total_orders = models.IntegerField(default=0, verbose_name="累計訂單")
    last_order_date = models.DateTimeField(
        null=True, blank=True, verbose_name="最後訂單日期"
    )

    # 積分有效期相關
    points_expiry_date = models.DateTimeField(
        null=True, blank=True, verbose_name="積分到期日"
    )
    last_points_update = models.DateTimeField(
        auto_now=True, verbose_name="最後積分更新時間"
    )

    # 會員信息
    join_date = models.DateTimeField(auto_now_add=True, verbose_name="加入日期")
    membership_number = models.CharField(
        max_length=20, blank=True, verbose_name="會員編號"
    )
    ...
    def add_points_from_order(self, order):
        ...
        try:
            ...
            CustomerActivity.record_points_earned(
                self.user, order.id, points_earned, order.total_price
            )

            msg = (
                f"用戶 {self.user.username} 訂單 #{order.id} 獲得 {points_earned} 積分"
            )
            logger.info(msg)
    ...
    def redeem_reward(self, reward_id):
        ...
        CustomerActivity.record_reward_redeemed(
            self.user, reward["name"], reward["points_required"]
        )

        logger.info(
            f"用戶 {self.user.username} 兌換獎勵: {reward['name']}, 消耗 {reward['points_required']} 積分"
        )
    ...
    def assign_membership_number(self):
        ...
        if not self.membership_number:
            ...
            self.save()
            logger.info(
                f"為用戶 {self.user.username} 分配會員編號: {self.membership_number}"
            )
    ...
    def should_assign_membership_number(self):
        ...
        return float(self.total_spent) >= 100 or self.total_orders >= 5
...
class CustomerCoupon(models.Model):
    ...
    coupon_type = models.CharField(
        max_length=20, choices=COUPON_TYPES, verbose_name="優惠類型"
    )
...
class CustomerActivity(models.Model):
    ...
    activity_type = models.CharField(
        max_length=20, choices=ACTIVITY_TYPES, verbose_name="活動類型"
    )
    ...
    def record_reward_redeemed(cls, user, reward_name, points_used):
        ...
        activity = cls.objects.create(
            ...
            metadata={"reward_name": reward_name, "points_used": points_used},
        )
        return activity
    ...
    def record_order_placed(cls, user, order_id, order_amount):
        ...
        activity = cls.objects.create(
            ...
            metadata={"order_id": order_id, "order_amount": float(order_amount)},
        )
        return activity
...
class RedeemedReward(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="redeemed_rewards"
    )
    ...
    used_at = models.DateTimeField(null=True, blank=True, verbose_name="使用時間")
    applied_order_id = models.IntegerField(
        null=True, blank=True, verbose_name="應用訂單ID"
    )
    ...
    def apply_reward_at_checkout(cls, user, reward_id, order_id=None):
        ...
        try:
            ...
            try:
                ...
                CustomerActivity.record_reward_redeemed(
                    user, reward_name, points_needed
                )
            except Exception:
                pass

            logger.info(
                f"用戶 {user.username} 在結帳時使用 {reward_name}，消耗 {points_needed} 積分，折扣 ${reward_info}"
            )
