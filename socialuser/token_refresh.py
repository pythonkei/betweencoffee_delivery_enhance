...
def refresh_google_token(social_token):
    ...
    try:
        ...
        if provider != 'google':
            logger.warning(f"refresh_google_token called for non-Google provider: {provider}")
            return False
        ...
        logger.info(f"Refreshing Google token for user {social_token.account.user.username}")
        ...
        if response.status_code == 200:
            ...
            if new_token:
                ...
                social_token.save()
                logger.info(f"Google token refreshed successfully, expires in {expires_in}s")
                return True
            else:
                logger.warning("Google token refresh response missing access_token")
                return False
        else:
            logger.error(f"Google token refresh failed: {response.status_code} {response.text[:200]}")
            return False
...
def refresh_facebook_token(social_token):
    ...
    try:
        ...
        if provider != 'facebook':
            logger.warning(f"refresh_facebook_token called for non-Facebook provider: {provider}")
            return False
        ...
        logger.info(f"Refreshing Facebook token for user {social_token.account.user.username}")
        ...
        if response.status_code == 200:
            ...
            if new_token:
                ...
                social_token.expires_at = timezone.now() + timedelta(seconds=expires_in)
                social_token.save()
                logger.info(f"Facebook token refreshed successfully, expires in {expires_in}s")
                return True
            else:
                logger.warning("Facebook token refresh response missing access_token")
                return False
        else:
            logger.error(f"Facebook token refresh failed: {response.status_code} {response.text[:200]}")
            return False
...
def check_and_refresh_token(social_token, min_remaining_minutes=5):
    ...
    if not social_token.expires_at:
        logger.info(f"Token for {social_token.account.provider} has no expiry, skipping refresh check")
        return True
    ...
    try:
        ...
        for account in social_accounts:
            try:
                ...
                logger.warning(f"No token found for {account.provider} account of user {user.username}")
