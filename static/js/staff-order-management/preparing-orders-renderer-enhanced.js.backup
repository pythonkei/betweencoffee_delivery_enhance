...
class EnhancedPreparingOrdersRenderer {
    ...
    createOrderElement(order, statusType) {
        ...
        orderDiv.className = 'order-item mb-4 p-4 rounded selectable';
        ...
        orderDiv.innerHTML = `
            <!-- 訂單類型徽章 -->
            <div class="order-type-badges-container mb-2">
                ${orderTypeBadges}
                ${statusBadge}
                ${countdownDisplay}
            </div>
            ...
            <div class="d-flex justify-content-between mb-3">
                <div>
                    ...
                    <p class="mb-1">
                        <strong>客戶:</strong> ${name} | <strong>電話:</strong> ${phone}
                    </p>
                    <p class="mb-2 text-muted">
                        <i class="far fa-clock mr-1"></i>訂單時間: ${orderTime}
                    </p>
                </div>
                <div class="text-right">
                    <span class="h5">$${parseFloat(totalPrice).toFixed(2)}</span>
                </div>
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center mt-3 pt-3 border-top">
                <div>
                    <span class="text-muted">
                        ${order.items_display || (order.items_count || 0) + '項商品'}
                    </span>
                </div>
                <div>
