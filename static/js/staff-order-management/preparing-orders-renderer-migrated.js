// static/js/staff-order-management/preparing-orders-renderer-migrated.js
// ==================== 遷移版製作中訂單渲染器 ====================
// 使用 CommonUtils 共用工具模塊，減少代碼重複

class MigratedPreparingOrdersRenderer {
    constructor() {
        console.log('🔄 初始化製作中訂單渲染器（遷移版）...');
    ...
    createOrderElement(order) {
        ...
        // 使用 CommonUtils 格式化時間
        const orderTime = window.CommonUtils.formatHKTime(order.created_at_iso || order.created_at);
        ...
        // 使用 CommonUtils 渲染訂單項目
        const orderItemsHTML = window.CommonUtils.renderOrderItems(order.items || order.coffee_items || []);
    ...
    bindEvents() {
        ...
        if (refreshBtn) {
            refreshBtn.addEventListener('click', () => {
                ...
                // 使用 CommonUtils 顯示 Toast
                window.CommonUtils.showToast('🔄 刷新中...', 'info');
                this.forceRefresh();
            });
        }
