...
class ApiResponse:
    ...
    def validation_error(
        cls, field_errors: Dict[str, List[str]], message: str = "數據驗證失敗"
    ) -> Dict:
        """
        創建驗證錯誤響應
    ...
    @classmethod
    def not_found(
        cls, resource_type: str, resource_id: Any, message: str = None
    ) -> Dict:
        """
        創建資源未找到錯誤響應
    ...
    @classmethod
    def permission_error(cls, resource: str, action: str, message: str = None) -> Dict:
        """
        創建權限錯誤響應
    ...
    def wrapper(request, *args, **kwargs):
        try:
            ...
            if isinstance(e, ValidationError):
                ...
                return JsonResponse(
                    ApiResponse.validation_error(field_errors), status=400
                )

            elif isinstance(e, PermissionDenied):
                return JsonResponse(
                    ApiResponse.permission_error("資源", "訪問"), status=403
                )
                ...
                if "matching query" in str(e):
                    resource_type = "記錄"
                return JsonResponse(
                    ApiResponse.not_found(resource_type, resource_id), status=404
                )
                ...
                if settings.DEBUG:
                    ...
                    return JsonResponse(
                        ApiResponse.internal_error(
                            str(e), traceback=error_traceback, request_id=request_id
                        ),
                        status=500,
                    )
                else:
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            ApiResponse.success(
                data={"user": {"id": 1, "name": "John"}}, message="用戶信息獲取成功"
            ),
            indent=2,
        )
    )
    ...
    print(
        json.dumps(
            ApiResponse.error(
                message="用戶不存在", code=1004, details={"user_id": 999}
            ),
            indent=2,
        )
    )
