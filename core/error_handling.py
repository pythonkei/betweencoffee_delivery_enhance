...
class ErrorHandler:
    ...
    def api_error_response(
        message, status=400, error_code=None, details=None, request=None
    ):
        """
        統一的API錯誤響應
        ...
        if not settings.DEBUG and isinstance(details, dict) and "traceback" in details:
    ...
    def log_error(context, exception, level="error", extra_data=None):
        ...
        log_data = {
            ...
            "context": context,
            "level": level.upper(),
            "module": inspect.currentframe().f_back.f_globals.get(
                "__name__", "unknown"
            ),
        }
        ...
        def wrapped_view(request, *args, **kwargs):
            try:
                ...
                if hasattr(e, "status_code"):
                    return ErrorHandler.api_error_response(
                        message=str(e), status=e.status_code, request=request
                    )
                elif "not found" in str(e).lower():
                    return ErrorHandler.api_error_response(
                        message="資源未找到", status=404, request=request
                    )
                elif "permission" in str(e).lower() or "權限" in str(e):
                    return ErrorHandler.api_error_response(
                        message="無權限訪問", status=403, request=request
                    )
                else:
                    return ErrorHandler.api_error_response(
                        message="伺服器內部錯誤", status=500, request=request
                    )
...
class ErrorContext:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_val:
            ...
            ErrorHandler.log_error(
                context=self.context_name, exception=exc_val, extra_data=self.extra_data
            )
...
def log_business_error(context, exception, **kwargs):
    ...
    return ErrorHandler.log_error(
        context, exception, level="warning", extra_data=kwargs
    )
